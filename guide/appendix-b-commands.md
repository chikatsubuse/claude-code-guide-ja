...
# ─── モデルと深さ ─────────────────────────────
/model                                           # モデル切替 (現セッションのみ; d でデフォルト変更)
...
claude plugin details <name>                     # コンポーネント一覧とトークンコスト表示 (v2.1.139〜)
...
/code-review high                                # effort 指定 (low/medium/high/xhigh/max)
/code-review high --comment                      # GitHub PR インラインコメントで報告
...
claude agents                                    # Agent View (全セッション一覧, Research Preview, v2.1.139〜)
claude agents --json                             # JSON でセッション一覧を出力 (スクリプト向け, v2.1.145〜)
