...
v2.1.118 より、`autoMode.allow` / `autoMode.soft_deny` / `autoMode.environment` に `"$defaults"` を含めることで、組み込みのデフォルトルールを維持しつつカスタムルールを追加できるようになった。従来は独自ルールを書くとデフォルトリストを置き換えていた ([CHANGELOG](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md#2-1-118))。

#### `autoMode.hard_deny` — 無条件ブロックルール (v2.1.136)

通常の Auto Mode 分類器はユーザーの意図や例外を考慮して判断するが、`hard_deny` は**ユーザーが許可しても絶対にブロック**するルールを追加する:

```json
// ~/.claude/settings.json
{
  "autoMode": {
    "hard_deny": ["Bash(rm -rf:*)", "Bash(curl * | bash:*)"]
  }
}
```

`soft_deny` との違い: `hard_deny` はユーザーの明示的な許可でも通らない。組織ポリシーで禁止すべき操作の強制に使う。
...
v2.1.133 で `sandbox.bwrapPath` (Linux) と `sandbox.socatPath` (Linux/WSL) が追加された。カスタムパスの bubblewrap / socat バイナリを指定する管理者向け設定:
