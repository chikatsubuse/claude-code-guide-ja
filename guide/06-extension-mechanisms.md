...
v2.1.120 で Skill の本文内で `${CLAUDE_EFFORT}` プレースホルダーが使えるようになった。現在の effort level を参照してレビューの深さを変えるなど、effort に応じた動的な指示が書ける。
...
- **MCP Tool hook** (`type: "mcp_tool"`): MCP サーバーのツールを直接呼び出す (v2.1.118)

この四段階により、「軽量チェック」「セマンティック評価」「深い分析」「外部ツール連携」を使い分けられます。

#### PostToolUse によるツール出力の置き換え (v2.1.121)

`PostToolUse` Hook で `hookSpecificOutput.updatedToolOutput` を返すと**全ツール**の出力を書き換えられる (v2.1.121 以前は MCP ツールのみ)。機密情報のマスキングやフォーマット変換に活用できる。
...
MCP サーバー設定に `alwaysLoad: true` を指定すると、そのサーバーのツールが tool-search の遅延ロードをスキップして常時利用可能になる。頻繁に使う MCP ツールに設定すると初回呼び出しの遅延がなくなる:
