...
#### Hook の新設定項目 (v2.1.139〜v2.1.141)

**`args: string[]`** (v2.1.139) — Command hook の exec 形式。シェルを介さずコマンドを直接起動するため、パスのクォート処理が不要になる:
...
**`continueOnBlock`** (v2.1.139) — `PostToolUse` フック専用オプション。`true` に設定すると、フックの拒否理由を Claude にフィードバックしてターンを継続させる (デフォルトは拒否でターン終了):
...
**`terminalSequence`** (v2.1.141) — フックの JSON 出力に含めると、デスクトップ通知・ウィンドウタイトル変更・ベル音などの端末エスケープシーケンスを発行できる。制御端末がないバックグラウンドエージェントでも機能する。
...
v2.1.139 から、MCP stdio サーバーの起動環境に `CLAUDE_PROJECT_DIR` が自動で渡されるようになった。フックと同じ変数名で、MCP サーバー内からプロジェクトルートを参照できる。`plugin.json` の MCP サーバー設定では `${CLAUDE_PROJECT_DIR}` をコマンド内で直接参照可能。
