...
- `/code-review` — コードレビュー・バグ検出 (v2.1.147: 旧 `/simplify` から改称)
- `/batch` — バッチ処理生成
- `/debug` — 構造化デバッグ
- `/loop` — 反復実行
...
#### exec form で引数を渡す (v2.1.139〜)

command hook に `args: string[]` フィールドを設定すると、シェルを介さずコマンドを直接起動できる。パス中のスペースや特殊文字をエスケープせずに使えるため、CI 環境や Windows でも安定して動作する:
...
```json
{
  "hooks": {
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "/path/to my script/run.sh",
        "args": ["--check", "$CWD"]
      }]
    }]
  }
...
#### PostToolUse の `continueOnBlock` オプション (v2.1.139〜)

`PostToolUse` フックに `continueOnBlock: true` を設定すると、フックがブロック (`exit 2`) を返した場合でも**ターンを継続**し、ブロック理由を Claude に伝える。「コマンド実行を止めるのではなく、Claude に修正させたい」場合に使う:
...
フックの JSON 出力に `terminalSequence` フィールドを含めると、制御端末がない環境 (ヘッドレス、CI、バックグラウンドセッション) でも**デスクトップ通知・ウィンドウタイトル変更・ベル音**などのターミナルシーケンスを送信できる。

#### `Stop` / `SubagentStop` フックの追加フィールド (v2.1.145〜)

`Stop` および `SubagentStop` フックの JSON 入力に `background_tasks`・`session_crons` フィールドが追加された。現在バックグラウンドで実行中のタスクや cron 設定を参照して、完了後の後処理を条件分岐させられる。

#### Hooks で effort level を参照する (v2.1.133)

Hooks の JSON 入力に `effort.level` フィールドが追加され、Bash ツールのコマンド内でも `$CLAUDE_EFFORT` 環境変数で現在の effort level を読み取れる。effort に応じたチェックの強度調整などに使える。
