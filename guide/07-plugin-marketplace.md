...
v2.1.139 から `/plugin` の **Discover・Browse** 画面でインストール前にプラグインが提供するコマンド・エージェント・Skill・Hooks・MCP/LSP サーバーの一覧を確認できるようになった。また v2.1.143 から Browse ペインに**セッションあたりの推定トークンコスト** (per-turn / per-invocation 見積もり) が表示される。

インストール済みプラグインの詳細は CLI からも確認できる:

```bash
claude plugin details <name>   # コンポーネント一覧 + 推定トークンコスト
```

v2.1.143 で**プラグイン依存関係の強制**が追加された。`claude plugin disable <name>` は他の有効なプラグインが依存している場合は拒否され、無効化順のヒントが表示される。`claude plugin enable <name>` は推移的な依存プラグインを自動で有効化する。
...
v2.1.142 から、プラグインのルートに `SKILL.md` を置き `skills/` サブディレクトリが無い場合でも、そのファイルが Skill として認識されるようになった。シンプルなシングル Skill プラグインの配布が容易になる。
...
| `claude plugin details <name>` | コンポーネント一覧 (Skills/Agents/Hooks/MCP) と推定トークンコストを表示 (v2.1.139) |
