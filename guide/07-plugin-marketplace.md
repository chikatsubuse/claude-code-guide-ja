...
v2.1.143 から、**Plugin 依存関係の強制** が追加された。`claude plugin disable <name>` は他の有効なプラグインが依存している場合に拒否し、無効化するための順序をコピー可能なヒントで提示する。逆に `claude plugin enable <name>` は推移的依存関係を自動で有効化する。

v2.1.143 から、`/plugin` マーケットプレイスの Browse ペインに **1 ターンあたり・1 回起動あたりのトークン推定コスト** が表示されるようになった。プラグインを選ぶ際のコスト判断材料として使える。
...
v2.1.142 から、`skills/` サブディレクトリを持たないプラグインでも、リポジトリルートに `SKILL.md` を置くだけで Skill として認識されるようになった。単一 Skill を提供するシンプルなプラグインの構成がより簡潔に書ける。
...
| `claude plugin details <name>` | プラグインのコンポーネント一覧・MCP サーバー・1 セッションあたりの推定トークンコストを表示 (v2.1.139) |
| `claude plugin disable <name>` | 他プラグインが依存している場合は拒否し、無効化順序のヒントを提示 (v2.1.143) |
| `claude plugin enable <name>` | 推移的依存関係を自動で有効化 (v2.1.143) |
