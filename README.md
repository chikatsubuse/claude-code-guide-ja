...
| バージョン | 変更内容 |
|---|---|
| **v2.1.147** | `/simplify` が **`/code-review`** に改称。旧 cleanup-and-fix 動作は廃止され、コードの正確性バグを effort 指定で検出するコマンドに刷新 (`/code-review high` など) |
| **v2.1.144** | `/model` が**現セッション限定**に変更。デフォルトモデルを変更するには model picker 内で `d` キーを押す |
